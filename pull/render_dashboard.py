...
<details class="glossary">
  ...
  <dl>
    ...
    <dd>Moneyline = which team our simulation (20,000+ Monte Carlo trials per game) thinks is more likely to win outright. Run line = the standard MLB spread (favorite -1.5, underdog +1.5) and which side is more likely to cover it. Total = the projected combined-runs line (always a half-run, like sportsbooks quote it) and whether the model leans over or under it. All three come from the same simulation, which uses each team's season-long scoring record (home/away split), the probable starters' season stats, actual bullpen quality plus recent bullpen fatigue, and -- once a lineup is confirmed -- a small adjustment for how those specific 9 hitters have actually been hitting lately.</dd>
  </dl>
</details>
"""
