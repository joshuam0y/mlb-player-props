...
function legProgressHtml(leg) {{
  // leg.line isn't validated to be positive at entry (a mistyped 0 passes
  // the add-bet form's own isNaN-only check) -- dividing by a zero/negative
  // line would otherwise render a nonsensical Infinity/NaN-width bar.
  if (leg.actual_value == null || !(leg.line > 0)) return '';
...
// Set while a bet's edit form is open (see the bet-edit-btn/cancel/save
// handlers below) -- renderBets() does a full innerHTML rebuild of the
// whole list, which would otherwise silently close/empty an open edit
// form (losing whatever the user was mid-typing) every time ANY leg's
// live value ticks. Now that the live poll re-renders on every value
// change rather than only on a HIT/MISS status flip (see
// applyLiveDataToBets()'s own comment), this used to fire every ~30s for
// the whole length of any live game a user had a bet on. Cleared (with an
// immediate catch-up render) the moment editing ends, so nothing skipped
// while suppressed is ever permanently lost.
let editingBetId = null;

function renderBets(bets) {{
  if (editingBetId != null) return;
...
document.getElementById('bets-list').addEventListener('click', async function (e) {{
  ...
  if (e.target.classList.contains('bet-delete-btn')) {{
    ...
    try {{
      ...
      // Deleting the bet currently being edited (clicking Delete instead of
      // Cancel) would otherwise leave editingBetId pointing at a bet that no
      // longer exists, permanently suppressing every future renderBets()
      // call for the rest of the page's life.
      if (betId === editingBetId) editingBetId = null;
    }} catch (err) {{
  ...
  if (e.target.classList.contains('edit-cancel-btn')) {{
    ...
    formEl.innerHTML = '';
    editingBetId = null;
    renderBets(currentBets); // catch up on anything that changed while editing was open
    return;
  }}
