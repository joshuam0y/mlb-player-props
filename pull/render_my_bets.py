...
let teamIndex = []; // [{{team_id, name, opponent, game_pk, side}}]
...
function searchTeams(text) {{
  const wanted = text.trim().toLowerCase();
  if (!wanted) return [];
  return teamIndex.filter(function (t) {{ return t.name.toLowerCase().indexOf(wanted) !== -1; }}).slice(0, 8);
}}
